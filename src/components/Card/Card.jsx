...
import './style.css'; // CSS stil dosyasını ekledim
...
const Card = ({ post }) => {
    ...
    const handleLike = async () => {
        if (token) {
            try {
                ...
                const response = await axios.patch(`${BASE_URL}api/posts/like/${post._id}`, null, {
                    headers: {
    ...
    const handleDislike = async () => {
        if (token) {
            try {
                ...
                const response = await axios.patch(`${BASE_URL}api/posts/dislike/${post._id}`, null, {
                    headers: {
                        token: `${token}` // Tokenni Headerga qo'shish
                    }
                });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${BASE_URL}api/users`, { email });
            if (response.data.success) {
                ...
                <div>
                    ...
                    <div className="modal-content">
                        <form onSubmit={handleSubmit}>
                            ...
                            <button type="button" onClick={handleModalClose}>Yopish</button>
                        </form>
                    </div>
                </div>
            )}
